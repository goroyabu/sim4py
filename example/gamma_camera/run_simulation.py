...
def define_detector():
    ...
    detector_construction\
        .SetParameter( "detector_center", 0, 0, -40, CLHEP.mm )\
        .SetParameter( "detector_normal", 0, 0, -1.0 )\
        .SetParameter( "world_size", 20, CLHEP.cm )

    collimator = wstrip.W4ParallelCollimator.Instance()
    
    collimator.SetParameter("hole_radius", 0.11, CLHEP.mm)\
        .SetParameter("septa_thickness", 0.50, CLHEP.mm)\
        .SetParameter("position", 0.0, 0.0, -38.0, CLHEP.mm)\
        .SetParameter("preset_name", "MINE1")

    detector_construction.AddGeometry( collimator );
